...
class ScanningPage extends Component {
  ...
  submitEmployeeRecord = (itemValue) => {
    ...
    if(itemValue.target.value.trim().length > 2 && itemValue.target.value.toUpperCase().slice(0, 3) === 'ACT'){
      this.setState({ UserID: '' })
      const nextSibling = document.querySelector(
        `input[name=ActivityID]`
      );
      nextSibling.focus();
      this.SaveAsActivity(itemValue.target.value)
      return;
    }
  ...
  SaveAsAccessID = (itemValue) => {
    this.setState({ UserID: itemValue})
    console.log('UserID: ', itemValue);
    if (this.state.ActivityID.trim().length === 0) {
      cogoToast.error("Please scan activity", { position: 'top-center', heading: 'Scan Activity' });
      return;
    }
    ...
    if (itemValue.trim().length < 9) {
      return;
    }
if(this.state.areaSeq.trim().length > 0){
    const employeeData = {
      "QueryType": "1",
      "ActiveEmployee": true,
      "sessionManagement": {
      ...
      .then((response) => {
        ...
        if (responseStatus.status === 200) {
          ...
          if (resultValue.Table[0].RecordID > 0) {
            ...
            cogoToast.error('', { position: 'top-center', heading: resultValue.Table[0].Column1, hideAfter: 5 });
            this.setState({ UserID: '',ActivityID:'' })
          }
  ...
  SaveAsActivity = (itemValue) => {
    this.setState({ ActivityID: itemValue.toUpperCase() })
    if (itemValue.target.value.trim().length === 0) {
      cogoToast.error("Activity ID not detected", { position: 'top-center', heading: 'Please scan your Activity ID' });
      return;
    }
    if(itemValue.target.value.trim().length > 8 && parseFloat(itemValue.target.value)>=0){
      this.setState({ ActivityID: '' })
      const nextSibling = document.querySelector(
        `input[name=UserID]`
      );
      nextSibling.focus();
      this.SaveAsAccessID(itemValue.target.value)
      return;
    }
    if (itemValue.target.value.trim().length < 7) {
      return;
    }
    if (itemValue.target.value.slice(0, 3) === 'ACT') {
      ...
      if(this.state.UserID.trim().length > 8){
        const employeeData = {
          "QueryType": "1",
          "ActiveEmployee": true,
          "sessionManagement": {
          ...
          "dtr": {
            ...
            "dateFrom": "",
            "dateTo": "",
            "areaCode": newData[0].AreaCode,
            "activityCode": newData[0].ActivityCode,
          },
          "employee": {}
        }
        console.log('employeeData: ', employeeData)
  ...
  submitActivityID = (itemValue) => {
    this.setState({ ActivityID: itemValue.target.value.toUpperCase() })
    if (itemValue.target.value.trim().length === 0) {
      cogoToast.error("Activity ID not detected", { position: 'top-center', heading: 'Please scan your Activity ID' });
      return;
    }
            ...
            <div class="card">
              <div class="card-body">
                <div class="input-group mb-3">
                  ...
                  <input type="text" className="form-control" name="ActivityID" onFocus={()=>this.setState({whatToScan: 'Activity ID'})}  value={this.state.ActivityID} placeholder="Scan Activity ID" aria-describedby="basic-addon1" autoFocus onChange={(itemValue) => this.submitActivityID(itemValue)} />
                </div>
                <div class="input-group mb-3">
                  <div class="input-group-prepend">
                    <span class="input-group-text" id="basic-addon1">WAMAS/GEEK Account</span>
                  </div>
                  <input type="text" className="form-control" name="WAMASGEEKAccount" onFocus={()=>this.setState({whatToScan: 'WAMAS/GEEK Account'})}  value={this.state.WAMASGEEKAccount} placeholder="Scan WAMAS/GEEK Account" aria-describedby="basic-addon1" onChange={(itemValue) => this.submitWAMASAccount(itemValue)} />
                </div>
                <div class="input-group mb-3">
                  <div class="input-group-prepend">
                    <span class="input-group-text" id="basic-addon1">Access ID</span>
                  </div>
                  <input type="text" className="form-control" name="UserID" onFocus={()=>this.setState({whatToScan: 'Access ID'})}  value={this.state.UserID} placeholder="Scan your Access ID" aria-describedby="basic-addon1" onChange={(itemValue) => this.submitEmployeeRecord(itemValue)} />
                </div>
              </div>
            </div>
