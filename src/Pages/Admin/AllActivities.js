...
class AllActivities extends Component {
    ...
    loadMoreDataActive() {
        const data = this.state.orgActivityList;
    ...
    loadMoreDataInactive() {
        const data = this.state.orgInActivityList;

        const slice = data.slice(this.state.offsetInactive, this.state.offsetInactive + this.state.perPage)
        this.setState({
            pageCount: Math.ceil(data.length / this.state.perPage),
            InActivityList: slice
        })

    }
    loadMoreDataAll() {
        const data = this.state.orgAllActivityList;
        const slice = data.slice(this.state.offset, this.state.offset + this.state.perPage)
        this.setState({
            pageCount: Math.ceil(data.length / this.state.perPage),
            AllActivityList: slice
        })
    ...
    getActivityList() {
        ...
        axios.post(API, postData, { withCredentials: false }).then((response) => {
            ...
            if (responseStatus.status === 200) {
                ...
                this.setState({ ActivityList: slice, pageCountActive: Math.ceil(responseValue.length / this.state.perPage), orgActivityList: responseValue })
        ...
        axios.post(API, postDataInactive, { withCredentials: false }).then((response) => {
            ...
            if (responseStatus.status === 200) {
                ...
                console.log('InActivityList: ', responseValue.length)
                this.setState({ InActivityList: slice, pageCountInactive: Math.ceil(responseValue.length / this.state.perPage), orgInActivityList: responseValue })
        ...
        axios.post(API, postDataAll, { withCredentials: false }).then((response) => {
            ...
            if (responseStatus.status === 200) {
                ...
                this.setState({ AllActivityList: slice, pageCount: Math.ceil(responseValue.length / this.state.perPage), orgAllActivityList: responseValue })
        ...
        axios.post(APIGetArea, postData, { withCredentials: false }).then((response) => {
            ...
            if (responseStatus.status === 200) {
                ...
                this.setState({ AreaList: responseValue.map((info) => ({ label: info.Description, value: info.SeqNo })), })
            }
    ...
    submitNewArea() {
        if (this.state.selectedArea === '') {
            cogoToast.error("Please Select Area", {
                position: 'top-center',
                heading: "Sorry, You can't proceed"
            });
            return;
        }
        if (this.state.Terminal.trim().length === 0) {
            cogoToast.error("Please Enter Terminal", {
                position: 'top-center',
                heading: "Sorry, You can't proceed"
            });
            return;
        ...
        const postData = {
            "QueryType": "9",
            "AreaCode": this.state.selectedArea.value,
            "ActivityCode": this.state.ActivityInfo.SeqNo,
            "TerminalID": this.state.Terminal.toUpperCase(),
            "sessionManagement": {
                "UserNo": AdminId,
                "ApiKey": "d2qrB3n4KGd1z9pAc1xkB5wiC5olGbqEyiAgoR"
            },
    ...
    AddActivity() {
        if (this.state.ActivityName.trim().length === 0) {
            cogoToast.error("Please Enter Activity", {
                position: 'top-center',
                heading: "Sorry, You can't proceed"
            });
            return;
        ...
        axios.post(API, postData, { withCredentials: false }).then((response) => {
            ...
            if (responseStatus.data.Value.Table[0].Column1 === 'New Activity has been added successfully.') {
                this.getActivityList()
                cogoToast.success(this.state.ActivityName + " Activity is Added ", {
                    position: 'top-center',
    ...
    submitUpdateActivity() {
        ...
        const recordId = parseInt(this.state.ActivityInfo.SeqNo.slice(3))
        ...
        axios.post(API, postData, { withCredentials: false }).then((response) => {
            ...
            if (responseStatus.status === 200) {
                this.getActivityList()
                cogoToast.success(this.state.ActivityInfo.Description + " is set to " + this.state.selectedStatus.label, {
                    position: 'top-center',
                    heading: "Sucessfully Updated"
                })

                this.setState({ ActivityName: '', AddModal: false, modalFor: '', selectedStatus: '', })
            } else {
                cogoToast.error(" ", {
                    position: 'top-center',
            ...
            this.setState({ ActivityName: '', AddModal: false, modalFor: '', selectedStatus: '', })
            console.log('Error Update Activity: ', error);
        });
    }
    ...
    async handleImageDownload() {
        ...
        cogoToast.loading('Downloading...').then(async () => {
            ...
            link.href = data;
            link.download = this.state.ActivityInfo.Description + '.jpg';
            ...
            <div className='font-face-arial'>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            ...
                                            <div class="input-group"
                                                ...
                                                <input type="text" class="form-control" placeholder="Activity" aria-label="Activity" aria-describedby="basic-addon2"
                                                    style={
                                                                            ...
                                                                            ActivityList.map((info, i) => (
                                                                                <tr key={i}>
                                                                                    ...
                                                                                    <td><AiOutlineQrcode size={30}
                                                                                        onClick={
                                                                                            () => this.setState({ ActivityInfo: info, AddModal: true, modalFor: 'Generate QR' })
                                                                                        } />
                                                                                        <BiPlusCircle size={30}
                                                                                            onClick={
                                                                                                () => this.setState({ ActivityInfo: info, AddModal: true, modalFor: 'Add to Terminal' })
                                                                                            } />
                                                                                        <BiEdit size={30}
                                                                                            onClick={
                                                                                                () => this.setState({ ActivityInfo: info, AddModal: true, modalFor: 'Update Activity' })
                                                                                            } />
                                                                                    </td>
                                                                                </tr>
                                                                            ))
                                                                            ...
                                                                            <tr>
                                                                                ...
                                                                                <td colspan="2">
                                                                                    <div className="col-12">
                                                                                        <p>Showing {this.state.currentPageActive === 0 ? '1' : this.state.currentPageActive * this.state.perPage + 1} to {this.state.currentPageActive + this.state.perPage > this.state.orgActivityList.length ? this.state.orgActivityList.length : (this.state.currentPageActive + 1) * this.state.perPage > this.state.orgActivityList.length ? this.state.orgActivityList.length : (this.state.currentPageActive + 1) * this.state.perPage} of {this.state.orgActivityList.length} entries</p></div>
                                                            ...
                                                            <div class="card-block table-border-style">
                                                                <div class="table-responsive">
                                                                    <table class="table table-hover">
                                                                        ...
                                                                        <tbody> {
                                                                            InActivityList.map((info, i) => (
                                                                                <tr key={i}>
                                                                                    <th scope="row">
                                                                                        {this.state.currentPageInactive === 0 ? i + 1
                                                                                            : (this.state.currentPageInactive * this.state.perPage) + (i + 1)
                                                                                        }</th>
                                                                                    <td>{
                                                                                        info.SeqNo
                                                                                    }</td>
                                                                            ...
                                                                            <tr>
                                                                                <td colspan="2">
                                                                                    <div className="col-12">
                                                                                        <p>Showing {this.state.currentPageInactive === 0 ? '1' : this.state.currentPageInactive * this.state.perPage + 1} to {this.state.currentPageInactive + this.state.perPage > this.state.orgInActivityList.length ? this.state.orgInActivityList.length : (this.state.currentPageInactive + 1) * this.state.perPage > this.state.orgInActivityList.length ? this.state.orgInActivityList.length : (this.state.currentPageInactive + 1) * this.state.perPage} of {this.state.orgInActivityList.length} entries</p></div>
                                                                    ...
                                                                    <table class="table table-hover">
                                                                        ...
                                                                        <tbody> {
                                                                            ...
                                                                            <tr>
                                                                                <td colspan="2">
                                                                                    <div className="col-12">
                                                                                        <p>Showing {this.state.currentPage === 0 ? '1' : this.state.currentPage * this.state.perPage + 1} to {this.state.currentPage + this.state.perPage > this.state.orgAllActivityList.length ? this.state.orgAllActivityList.length : (this.state.currentPage + 1) * this.state.perPage > this.state.orgAllActivityList.length ? this.state.orgAllActivityList.length : (this.state.currentPage + 1) * this.state.perPage} of {this.state.orgAllActivityList.length} entries</p></div>
                ...
                <Modal show={AddModal}>
                    <Modal.Header closeButton
                        ...
                        <Modal.Title>{
                            modalFor === 'Add to Terminal' ? 'Add Activity to Terminal' : modalFor === 'Generate QR' ? this.state.ActivityInfo.Description + ' QR Code' : 'Update Status'
                        }</Modal.Title>
                    </Modal.Header>
                            ...
                            <div class="card-body text-center" id="print">
                                <div style={{ border: '3px solid black', borderRadius: 20, padding: 10 }} >
                                    ...
                                    <QRCode
                                        ...
                                        value={this.state.ActivityInfo.SeqNo}
                                        viewBox={`0 0 256 256`}
                                    />

                                    <p style={{ marginTop: 20, fontSize: 30, color: 'black', fontWeight: 'bold' }}>{this.state.ActivityInfo.Description}</p>
                                </div>
                    ...
                    {modalFor === 'Generate QR' ? null : <Modal.Footer>
